...
fname='/global/project/projectdirs/dasrepo/vpa/supernova_cnn/data/gathered_data/gathered_filepaths.csv'
### Write column names to file 
with open (fname,'w') as f: f.write('ID,filename,file path\n')


#####################
### Use os.walk() to get full path for every .gif file in the folder (recursive)
dir_name=data_dir
...
for count,(root, dirs,files) in enumerate(os.walk(dir_name)):
    for file in files:
        if file.endswith('.gif'):
            id=int(file.split('.gif')[0][4:])    # Extract the ID from the file name
